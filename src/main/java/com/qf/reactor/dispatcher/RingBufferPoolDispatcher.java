...
public final class RingBufferPoolDispatcher extends AbstractSingleThreadDispatcher {
	...
	public RingBufferPoolDispatcher(String name,IConfig config) throws Exception {
		...
		available =available>config.getCorePoolSize()?config.getCorePoolSize():available;
		
		if(config.getQueueSize() >0){
			this.eventHandler=new ThreadPoolExecutor(available,config.getCorePoolSize(),
		...
		this.disruptor.handleEventsWith(new EventHandler<RingBufferTask>() {
			...
			public void onEvent(RingBufferTask task, long sequence, boolean endOfBatch) throws Exception {
				...
				task.recycle();
				eventHandler.execute(new Runnable() {
					@Override
					public void run() {
